...
=== "Bangla"
    ...
    1. **প্রসিডিউরাল ল্যাঙ্গুয়েজ (Procedural Language):** সি কে বলা হয় একটা প্রসিডিউরাল ল্যাঙ্গুয়েজ (Procedural Language)। এটার মেইন কনসেপ্ট হলো তোমার একটা বিশাল বড় দানব সাইজের প্রোগ্রামকে (Gigantic program) ছোট ছোট টুকরা প্রোগ্রামে ভাগ করে ফেলা, যেগুলোকে প্রসিডিউর বা ফাংশন (Procedures or Functions) বলে। সব পড়ালেখা বা কাজ যেমন এক রাতে ক্র্যাম (Cram) বা মুখস্থ করা যায় না, ঠিক তেমনি সি ল্যাঙ্গুয়েজও তোমাকে একটা বড় কাজকে ছোট ছোট টাস্কে ডিভাইড করে আলাদাভাবে কাজ করার কমফোর্ট দেয়। সি-তে যা আছে তার সবই আসলে ফাংশনের খেলা (Set of procedures or functions), আর এটাই এই ল্যাঙ্গুয়েজের মেইন ইউএসপি বা ফিচার।
    2. **অ্যাবস্ট্রাকশনের লেভেল (Degree of Abstraction):** হাই লেভেল ল্যাঙ্গুয়েজ (High level language) আর লো লেভেল ল্যাঙ্গুয়েজের (Low level language) মেইন ডিফারেন্সটা আসে এই অ্যাবস্ট্রাকশনের লেভেল (Degree of Abstraction) থেকে। অ্যাবস্ট্রাকশন মানে হলো সিস্টেমের ভেতরের হাবিজাবি ইন্টারনাল ডিটেইলস হাইড করে রাখা (Hiding system level details)। যত বেশি অ্যাবস্ট্রাকশন থাকবে, ইউজারের প্যারা তত কম আর জিনিসটা তত বেশি ইউজার ফ্রেন্ডলি (User friendliness) হবে, কারণ ভেতরে আন্ডার-দ্য-হুড কী হচ্ছে তা নিয়ে আমাদের একদমই ভাবতে হয় না। যেমন ধরো একটা সিম্পল টেক্সট এডিটর (Text editor), তুমি জাস্ট টাইপ করো আর সেভ বাটন (Save button) চাপো, এটি হার্ডডিস্কের ঠিক কোথায় আর কেমনে স্টোর হচ্ছে তা নিয়ে তোমার বিন্দুমাত্র টেনশন করতে হয় না। হাই লেভেল ল্যাঙ্গুয়েজের এক্সাম্পল হলো COBOL, FORTRAN, C++, LIPS, Pascal আর Prolog। অন্য দিকে লো লেভেল অ্যাবস্ট্রাকশন মানে ইউজারের জান শেষ, কম্পিউটারের একদম প্রতিটা ইন্টারনাল ডিটেইলস তোমার মুখস্থ থাকা লাগবে। যেমন অ্যাসেম্বলি ল্যাঙ্গুয়েজ (Assembly language)।
    ...
    * সি কোডের ভেতরেই সরাসরি অ্যাসেম্বলি কোড (Assembly code) ঢুকিয়ে দেওয়া।
    ...
    কোড এফিশিয়েন্টলি লেখার আর রান করার জন্য কোড ব্লকস আইডিই (Code Blocks IDE) এর মতো একটা এনভায়রনমেন্ট বা সমন্বিত উন্নয়ন পরিবেশ (Integrated Development Environment) লাগে। কোডিংয়ের আসল মজা তখনই আসে যখন তুমি নিজে কোড টাইপ করে হাত নোংরা করবে (Get your hands little dirty)। চলো একটা বেসিক সি প্রোগ্রামের প্রতিটা লাইনের পোস্টমর্টেম (Depict) করা যাক:
    ...
    এই যে `stdio.h` দেখছ, এটার ফুল ফর্ম হলো স্ট্যান্ডার্ড ইনপুট আউটপুট ফাইল (Standard input output file)। আর ফাইলে যে ডট এইচ `.h` আছে, ওটা দিয়ে বোঝায় এটা একটা হেডার ফাইল (Header file)। এই হেডার ফাইলগুলোতে আমরা যেসব ফাংশন ইউজ করব সেগুলোর জাস্ট একটা প্রোটোটাইপ বা ঘোষণা (Prototypes or Declaration) দেওয়া থাকে। যেমন ধরো `stdio.h` ফাইলে `printf` আর `scanf` ফাংশনের ডিক্লেয়ারেশন থাকে। `printf` এর কাজ হলো ব্র্যাকেট বা প্যারেন্থেসিসের (Parenthesis) ভেতরের কন্টেন্ট স্ক্রিনে প্রিন্ট করা, আর `scanf` কাজ করে রান টাইমে (Run time) ইউজারের থেকে ইনপুট নেওয়ার জন্য। তুমি কোডে কী কী ফাংশন ইউজ করতেছ তা কম্পাইলারকে আগেভাগে জানানোর জন্য হেডার ফাইল ইনক্লুড করা মাস্ট।
    4. **ফাংশন আর ভ্যারিয়েবলসের চিল মোড (Functions and Variables):**
    সি-তে তুমি যা-ই লেখো না কেন, ঘুরিফিরি দুইটা মেইন এলিমেন্ট থাকবেই:
    ...
    * **ফাংশনের নাম (Function Name):** কোডের রিডাবিলিটি বা পঠনযোগ্যতা (Readability) জোস রাখার জন্য যেকোনো মানানসই নাম দেওয়া যায় যেমন যোগের জন্য `add` নামটা বেস্ট, কিন্তু এই `main` ফাংশনটা আগে থেকেই ফিক্সড বা প্রি-সংজ্ঞায়িত (Pre defined), এটার নাম তুমি কোনোদিনও চেঞ্জ করতে পারবে না।
