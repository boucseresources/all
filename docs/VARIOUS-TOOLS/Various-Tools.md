# Tools - Some important tools for study

 এখানে আমাদের বানানো কিছু টুলস রয়েছে, যেগুলো দিয়ে ফ্রিতেই ব্যবহার করে অনেক ধরনের simulation সহ আরও অনেকভাবে আমাদের life কে easy করা যাবে। 

 ---

!!! info "KMap Generator"
    
    [![KMap Generator](image.png)](/VARIOUS-TOOLS/abc-kmap-generator/index.html)



<!-- prettier-ignore-start -->
??? folder "Folder অনুযায়ী দেখতে Click করুন!"
    <div class="grid cards" markdown>
     

    - [![KMap Generator](image.png)](/VARIOUS-TOOLS/abc-kmap-generator/index.html) 
    - [![Image 2](docs/assets/folder-images/01.png)](/bou-resources/bou-cse/12-semester/)
    - [![Image 3](docs/assets/folder-images/1.png)](/bou-resources/bou-cse/21-semester/)
    - [![Image 5](docs/assets/folder-images/2.png)](/bou-resources/bou-cse/22-semester/)
    - [![Image 6](docs/assets/folder-images/3.png)](/bou-resources/bou-cse/31-semester/)
    - [![Image 7](docs/assets/folder-images/4.png)](/bou-resources/bou-cse/32-semester/)
    - [![Image 8](docs/assets/folder-images/5.png)](/bou-resources/bou-cse/41-semester/)
    - [![Image 4](docs/assets/folder-images/6.png)](/bou-resources/bou-cse/42-semester/)

    </div>

<!-- prettier-ignore-end -->


